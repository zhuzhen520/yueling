...
class Redis
{
    ...
    public function __construct($options = [])
    {
        ...
        $this->handler->$func(Env::get('redis.host'), Env::get('redis.port'), Env::get('redis.timeout'));
        if ('' != Env::get('redis.password')) {
            $this->handler->auth(Env::get('redis.password'));
        }
        if (0 != Env::get('redis.database')) {
            $this->handler->select(Env::get('redis.database'));
        }
        if (Env::get('redis.prefix')) {
            $this->handler->setOption(\Redis::OPT_PREFIX, Env::get('redis.prefix'));
        }
    }
    ...
    /**
     * 创建消息监听通道消息订阅。
     * @param arrary $patterns 传入的通道数组
     * @param $callback 回调方法
     * @return string
     */
    public function subscribe($channels = array(), $callback)
    {
        $this->handler->subscribe($channels, $callback);
    }
    /**
     * 创建消息监听通道消息订阅。
     * @param arrary $channels 传入的通道名称
     * @param $message 发布信息
     * @return string
     */
    public function publish($channels, $message)
    {
        $this->handler->publish($channels, $message);
    }
