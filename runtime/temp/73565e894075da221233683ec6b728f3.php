<?php /*a:3:{s:59:"/www/wwwroot/yueling/application/admin/view/user/index.html";i:1567821037;s:62:"/www/wwwroot/yueling/application/admin/view/public/header.html";i:1567821037;s:62:"/www/wwwroot/yueling/application/admin/view/public/footer.html";i:1567821037;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
...
<div class="pd-20">
    <form action="" method="get">
        <div class="text-c">
            ...
            <input type="text" class="input-text" style="width:250px" placeholder="请输入用户手机号" name="mobile" value="<?php echo input('mobile'); ?>">
            <span class="select-box inline">
				<select id="types" name="type" class="select">
					<option value="0">=用户搜索=</option>
						<option value="1" <?php if(input('get.type') == 1): ?>selected<?php endif; ?> >直推</option>
                    <option value="2" <?php if(input('get.type') == 2): ?>selected<?php endif; ?> >上级</option>
				</select>
            </span>
                    ...
                    <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$v): ?>
                        <tr class="text-c" >
                            <td style="padding: 0">
                                ...
                                <?php else: ?>
                                <img src="<?php echo htmlentities($v['avatar']); ?>" style="width: 40px;height: 40px;" onclick="showimg('<?php echo htmlentities($v['avatar']); ?>')">
                                <?php endif; ?>
                            ...
                            <td><?php echo htmlentities($v['parent_id']); ?></td>
                            <td style="padding-left: 30px">
                               <span style="float: left">余额:<?php echo floatval($v['money']); ?></span><br>
                               <span style="float: left">豆豆:<?php echo floatval($v['cz_money']); ?></span><br>
                            </td>
                            <td>
                                会员：<?php echo htmlentities($v['zhi_vip']); ?><br>
                                普通：<?php echo htmlentities($v['zhi_p']); ?><br>
                                总计：<?php echo htmlentities($v['zhi_x']); ?>
                            </td>
                            <td><?php echo htmlentities($v['address']); ?></td>

                            <td><?php echo htmlentities($v['agent_level']); ?><br><?php echo htmlentities($v['agent_id']); ?></td>
                            <td>注册时间:<span style="font-size: 12px"><?php echo htmlentities($v['created']); ?></span><br>
                                <?php if(!empty($v['level_end_time'])): ?>
                                <span style="color: red;">会员时间:</span>
                                   <?php echo htmlentities($v['level_start_time']); ?>-<br>
                               　　　　 <?php echo htmlentities($v['level_end_time']); ?>

                                <?php endif; ?>
                            </td>
                            <td>
                                <?php switch($v['level']): case "0": ?> <span class="label label-default radius">普通用户</span> <?php break; case "1": ?> <span class="label label-primary radius">会员</span> <?php break; ?>
                                <?php endswitch; ?>
                            </td>
                            <td>
                                 <img src="/static/Admin/images/icon_<?php echo $v['status']!=1 ? 'right' : 'error'; ?>_s.png" style="cursor:pointer;" onclick="tog_status(this,<?php echo htmlentities($v['id']); ?>,<?php echo htmlentities($v['status']); ?>,'user','status')">
                            </td>
                            <td>
                                <a style="text-decoration:none" class="btn btn-secondary radius" onClick="new_open('充值','/user/userrecharge?id=<?php echo htmlentities($v['id']); ?>',800,400)" href="javascript:void(0);" title="充值"><i class="Hui-iconfont">充值</i></a>
                                <a style="text-decoration:none" class="btn btn-success ml-5" onClick="new_open('添加/编辑','/user/useradd?id=<?php echo htmlentities($v['id']); ?>')" href="javascript:void(0);" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
