<?php /*a:3:{s:65:"/www/wwwroot/yueling/application/admin/view/product/category.html";i:1567821037;s:62:"/www/wwwroot/yueling/application/admin/view/public/header.html";i:1567821037;s:62:"/www/wwwroot/yueling/application/admin/view/public/footer.html";i:1567821037;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
...
<div class="pd-20">
	<div class="cl pd-5 bg-1 bk-gray mt-20">
		...
		<span class="r">共有数据：<strong id="count"><?php echo count($list); ?></strong> 条</span>
 </div>
	<div class="mt-20">
		<form action="<?php echo url('category'); ?>" method="post" id='sort-form'>
			<table class="table table-border table-bordered table-bg table-hover table-sort">
				...
				<tbody>
					<?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$v): ?>
						<tr class="text-c" id="tr<?php echo htmlentities($v['cat_id']); ?>">
							<td><?php echo htmlentities($v['cat_id']); ?></td>
							<td style="padding: 0">
								<?php if(!empty($v['thumb'])): ?>
								<img src="<?php echo htmlentities($v['thumb']); ?>" style="width: 60px;height: 60px;" onclick="showimg('<?php echo htmlentities($v['thumb']); ?>')">
								<?php else: ?>
								<div  style="width: 60px;height: 60px;"></div>
								<?php endif; ?>
							</td>
							<td class="text-l"><?php echo str_repeat('　　►', $v['level']); ?>&nbsp;<?php echo htmlentities($v['cat_name']); ?></td>
							<td>
								<img src="/static/Admin/images/icon_<?php echo $v['is_show']!=0 ? 'right' : 'error'; ?>_s.png" style="cursor:pointer;" onclick="toggle_status(this,<?php echo htmlentities($v['cat_id']); ?>,<?php echo htmlentities($v['is_show']); ?>,'product_category')">
							</td>
							<td>
 								<a style="text-decoration:none" class="btn btn-success ml-5" onClick="new_open('添加/编辑','/product/category_add?cat_id=<?php echo htmlentities($v['cat_id']); ?>')" href="javascript:void(0);" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a>
								<a style="text-decoration:none" class="btn btn-danger ml-5" onclick="member_del(this,<?php echo htmlentities($v['cat_id']); ?>)" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.post("<?php echo url('category_del'); ?>",{id:id},function(data){
			if(data.status == 'y'){
				//删除自己
				$(obj).parents("tr").remove();
				//数量赋值
