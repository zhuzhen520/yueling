<?php /*a:3:{s:63:"/www/wwwroot/yueling/application/admin/view/user/user_auth.html";i:1568014175;s:62:"/www/wwwroot/yueling/application/admin/view/public/header.html";i:1567821037;s:62:"/www/wwwroot/yueling/application/admin/view/public/footer.html";i:1567821037;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    ...
    <div class="mt-20">
        ...
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            <tr class="text-c" id="tr<?php echo htmlentities($v['user_id']); ?>">
                ...
                <td><?php echo htmlentities($v['card_no']); ?></td>
                <!--<td style="padding: 0"><img src="<?php echo htmlentities($v['obverse']); ?>" style="width: 70px;height: 40px;" onclick="showimgmm('<?php echo htmlentities($v['obverse']); ?>')"></td>-->
                <td style="padding: 0"><img src="<?php echo htmlentities($v['reverse']); ?>" style="width: 70px;height: 40px;" onclick="showimgmm('<?php echo htmlentities($v['reverse']); ?>')"></td>
                <td style="padding: 0"><img src="<?php echo htmlentities($v['userface']); ?>" style="width: 60px;height: 60px;" onclick="showimgww('<?php echo htmlentities($v['userface']); ?>')"></td>
                <td>
                    <?php if(intval($v['score']) == 0): ?>
                   	<a style="text-decoration:none" class="btn btn-primary ml-5" onclick="user_auth_update(this,<?php echo htmlentities($v['id']); ?>)"  href="javascript:void(0);" title="刷新">刷新</a>
                    <?php else: ?>
                   	<?php echo htmlentities($v['score']); ?>%
                    <?php endif; ?>
                </td>
...
<script type="text/javascript">
    ...
    function user_auth_update(obj,id){
        layer.confirm('刷新相似度',function(index){
            ...
            $.post("<?php echo url('user_auth_update'); ?>",{id:id},function(data){
    ...
    function member_sta(obj,id,status){
        layer.confirm('确认要修改吗？',function(index){
            $.post("<?php echo url('user_auth_sta'); ?>",{id:id,status:status},function(data){
                layer.msg(data.info,{icon:1,time:3000});
                window.location.reload();
            },'json');
        });
