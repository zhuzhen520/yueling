<?php /*a:3:{s:65:"/www/wwwroot/yueling/application/admin/view/user/money_apply.html";i:1567821037;s:62:"/www/wwwroot/yueling/application/admin/view/public/header.html";i:1567821037;s:62:"/www/wwwroot/yueling/application/admin/view/public/footer.html";i:1567821037;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
...
<div class="pd-20">
	<form action="" method="get">
		<div class="text-c">
			...
			<input type="text" class="input-text" style="width:250px" placeholder="姓名|申请id" value="<?php echo input('get.word'); ?>" name="word">
			<button type="submit" class="btn btn-success radius"><i class="Hui-iconfont">&#xe665;</i> 搜索</button>
		</div>
	</form>

	<div class="cl pd-5 bg-1 bk-gray mt-20">
		<span class="r" style="float: right;">共有数据：<strong id="count"><?php echo htmlentities($count); ?></strong> 条</span>
	</div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-hover table-bg table-sort">
			...
			<tr class="text-c" id="tr<?php echo htmlentities($v['id']); ?>">
				...
				<td>
					<?php switch($v['card_type']): case "1": ?> <span class="label label-success radius">支付宝</span> <?php break; case "2": ?> <span class="label label-primary radius">微信</span> <?php break; case "3": ?> <span class="label label-default radius">银行卡</span> <?php break; ?>
					<?php endswitch; ?>
				</td>
				<td> <?php echo htmlentities($v['card']); ?></td>
				<td>
					<?php switch($v['card_type']): case "1": ?> <img src="<?php echo htmlentities($v['img']); ?>" style="width: 40px;height: 40px;" onclick="showimg('<?php echo htmlentities($v['img']); ?>')"><?php break; case "2": ?> <img src="<?php echo htmlentities($v['img']); ?>" style="width: 40px;height: 40px;" onclick="showimg('<?php echo htmlentities($v['img']); ?>')"><?php break; case "3": ?> <span class="label label-default radius"><?php echo htmlentities($v['khh']); ?><br/><?php echo htmlentities($v['khzh']); ?></span> <?php break; ?>
					<?php endswitch; ?>
				</td>

				<td> <?php echo htmlentities($v['money']); ?></td>
				<td> <?php echo htmlentities($v['usdt']); ?></td>
				<td> <?php echo htmlentities($v['all_money']); ?></td>
				<td><?php echo htmlentities($v['add_time']); ?></td>
				<td>
					<?php switch($v['status']): case "0": ?> <span class="label label-default radius">未审核</span> <?php break; case "2": ?> <span class="label label-primary radius">已通过</span> <?php break; case "1": ?> <span class="label label-default radius">已驳回</span> <?php break; ?>
					<?php endswitch; ?>
				</td>
				<td class="td-manage">
					<?php switch($v['status']): case "0": ?>
						<a style="text-decoration:none" class="btn btn-primary ml-5" onclick="member_sta(this,<?php echo htmlentities($v['id']); ?>,2)"  href="javascript:void(0);" title="通过"><i class="Hui-iconfont">&#xe615;</i></a>
						<a style="text-decoration:none" class="btn btn-warning ml-5" onclick="member_sta(this,<?php echo htmlentities($v['id']); ?>,1)" title="驳回"><i class="Hui-iconfont">&#xe60b;</i></a>
					<?php break; ?>
					<?php endswitch; ?>
				</td>
			</tr>
...
<script>
	...
	function member_sta(obj,id,status){
		layer.confirm('确认操作吗？',function(index){
			...
			$.post("<?php echo url('money_apply_status'); ?>",{id:id,status:status},function(data){
				if(data.status == 'y'){
					layer.msg(data.info,{icon:1,time:1000});
					window.location.reload();
				}else{
